	...
	<li data-id="company_<?= $company->id ?>">
		...
		<div class="list_search-result-btn">
			<? if(is_null($company->followUserId)) : ?>
				<a href="<?= Request::generateUri('companies', 'followFromSearch', $company->id); ?>"  onclick="return web.ajaxGet(this);" class="icons i-accept icon-text btn-icon" ><span></span>Follow</a>
			<? else: ?>
				<a href="<?= Request::generateUri('companies', 'followFromSearch', $company->id); ?>"  onclick="return web.ajaxGet(this);" class="icons i-close icon-text btn-icon" ><span></span>Unfollow</a>
			<? endif; ?>
		</div>
