...
class Form_Companies_EditCompany extends Form_Main
{
	public function __construct($company)
	{
		$form = new Form('editcompany', false, Request::generateUri('companies', 'edit', $company->id));
...
//			->attribute('class', 'form-html');
		...
		$viewLogo = View::factory('pages/companies/block-ava_logo', array(
			'company' => $company
		));

		$form->html('company_logo', 'Logo', $viewLogo)
			->attribute('class', 'form-html');



//		$form->html('name_label', false, '<b>Company name:</b>')
//			->attribute('class', 'form-html');
		...
		$form->text('name', 'Company name')
			...
			->rule(function ($field) use ($company) {
				$isCompany = Model_Companies::checkIsRegistredByNameWithoutId($field->value, $company->id);
...
//			->attribute('class', 'form-html');
		...
		$form->select('industry', array('' => '') + t('industries'), 'Company industry')
...
//		$form->html('type_label', false, '<b>Company type:</b>')
//			->attribute('class', 'form-html');

		$form->select('type', array('' => '') + t('company_type'), 'Company type')
			->attribute('class', 'bootstripe')
...
//			->attribute('class', 'form-html');
		...
		$form->select('size', array('' => '') + t('company_number_of_employer'), 'Number of employer')
			->attribute('class', 'bootstripe')
...
//		$form->html('phone_label', false, '<b>Phone:</b>')
//			->attribute('class', 'form-html');
...
//		$form->html('email2_label', false, '<b>Company associate email:</b>')
//			->attribute('class', 'form-html');

		$form->text('email2', 'Company associate email')
...
//		$form->html('company_cover_label', false, '<b>Cover:</b>')
//			->attribute('class', 'form-html');

		$viewCover = View::factory('pages/companies/block-ava_cover', array(
			'company' => $company
		));

		$form->html('company_cover', 'Cover', $viewCover)
			->attribute('class', 'form-html');



//		$form->html('description_label', false, '<b>Company description:</b>')
//			->attribute('class', 'form-html');
		...
		$form->html('delete', '<a class="btn-roundbrown" href="' . Request::generateUri('companies', 'remove', $company->id) . '" onclick="return box.confirm(this);" title="Remove company">Remove</a>');
		$form->html('cancel', '<a class="btn-roundbrown" href="' . Request::generateUri('companies', 'index', $company->id) . '">Cancel</a>');
	...
	public function setValues($company)
	{
