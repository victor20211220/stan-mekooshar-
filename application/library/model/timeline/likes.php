...
class Model_Timeline_Likes extends Model{

	protected static $table = 'timeline_likes';

	public static function checkIsset($user_id, $timeline_id)
	{
		$timelineLike = self::query(array(
			'where' => array('user_id = ? AND parentTimeline_id = ?', $user_id, $timeline_id)
		))->fetch();

		if(!is_null($timelineLike)) {
			$timelineLike = self::instance($timelineLike);
			Model_User::addUserIdByKey($timelineLike, 'user_id');
		} else {
			$timelineLike = false;
		}
		return $timelineLike;
	...
	public static function getListByTimeline($timeline)
	{
		...
		$results = self::getList(array(
			...
			'join' => array(
				array(
					'table' => 'users',
					'where' => array('users.id = timeline_likes.user_id')
				),
				array(
					'type' => 'left',
