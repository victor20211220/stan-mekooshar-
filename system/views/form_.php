			...
			if (false === $el instanceof System_Form_Field_Hidden) {
				...
				if (false === $inline) {
					...
					echo '<li'
						...
						. '>'
						. '<label for="' . $form->attributes['id'] . '-' . $el->name . '"' . $labelWidth . '>'
						. ($el->label ? $el->label : '&nbsp;')
						. ($el->hint  ? '<sup title="' . $el->hint . '">'. $form->hintHtml . '</sup>' : '')
						. ($el->required ? '&nbsp;<em>*</em>' : '&nbsp;')
						. ($el->label ? $form->labelSeparator : '')
						. '</label><div class="autoform-element">';
				}
						...
						if ($errors) {
							...
							foreach ($errors as $i) {
								...
								foreach ($i->errors as $msg) {
									echo '<li' . ($v ? ' style="padding-right: ' . $v . '"' : '') . '><label for="' . $id . '">' . $msg . '</label></li>';
								}
							}
							echo '</ul>';
						}
...
if (null === $render || $render === '__header__') {
	...
	if ($form->displayWarnings) {
		$list = array();
		foreach($form->elements as $el) {
			if (!empty($el->warnings)) {
				$list[] = $el;
			}
		}
		if ($list) {
			echo '<ul class="autoform-warnings">';
				$id = $el->composeId();
				foreach ($el->warnings as $msg) {
					echo '<li><label for="' . $id . '">' . $msg . '</label></li>';
				}
			echo '</ul>';
		}
