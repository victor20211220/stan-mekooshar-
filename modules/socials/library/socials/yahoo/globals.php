...
function callcontact_yahoo($consumer_key, $consumer_secret, $guid, $access_token, $access_token_secret, $usePost=false, $passOAuthInHeader=true, $count = 1000)
{
	$retarr = array();  // return value
	$response = array();
...
//			dump($yahoo_array->contacts->contact, 1);
			// echo "<pre/>";
			foreach($yahoo_array->contacts->contact as $values){
				if(isset($values->fields[1]->value) && !empty($values->fields[1]->value)) {
					$email = $values->fields[1]->value;
				} elseif(isset($values->fields[0]->value->givenName) && !empty($values->fields[0]->value->givenName)) {
					$email = $values->fields[0]->value->givenName;
				}
...
function callprofile_yahoo($consumer_key, $consumer_secret, $guid, $access_token, $access_token_secret, $usePost=false, $passOAuthInHeader=true, $count = 1000)
{
	$retarr = array();  // return value
	$response = array();

	$url = 'https://social.yahooapis.com/v1/users.guid(' . $guid . ')/profile';
	...
	if (! empty($response)) {
		...
		if ($body) {
			...
			return $yahoo_array->profiles->profile[0];
		}
	}
}
