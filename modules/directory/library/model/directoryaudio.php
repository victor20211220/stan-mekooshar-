...
class Model_Directoryaudio extends Model_Directoryfile
{
	protected static $table = 'directoryAudios';
	
	public static $path = 'content/directory/music/';
	...
	{
		return '/download/audio/' . $item->section . '/' . $item->alias . '/';
	}
	
	public static function removeItem($item, $onlyFiles = false)
	{
		$dir = self::dir($item->alias, $item->section);
		$fileName = self::dir($item->alias, $item->section) . $item->alias . '.' . $item->ext;
